...
from model.ernie.modeling_ernie import append_name, _build_linear
...
class Model(Encoder):
    ...
    def forward(self, *args, **kwargs):
        ...
        logit_p = self.label_classifier(logit)  # [batch_size, line_num, num_labels]

        label = input_data.get('label')
        logit = P.argmax(logit_p, axis=-1)
        ...
        return {'logit': selected_logit, 'label': selected_label,
                'logit_prim': logit, 'label_prim': label, 'mask': mask, 'scores': logit_p}
